...
export class ProductsService {
  ...
  async create(
    ...
    if (await Product.findOne({ where: { slug } }))
      returnErrorResponse('Product name already exists');
  ...
  async products(
  ...
  ): Promise<{ products: Product[]; total_rows: number }> {
    ...
    if (user.role === Role.ADMIN) {
      const [products, count] = await Product.findAndCount({
        relations: { images: true, vendor: true },
        select: { vendor: { name: true, email: true, logo: true } },
        where: where,
  ...
  async remove(
    ...
    if (!product) returnErrorResponse('Product does not exist');
    if (vendor != product.vendor_id && user.role !== Role.ADMIN)
      returnErrorResponse('Unauthorized');
