...
export class UsersService {
  ...
  async getUsers(user: User, pagination: PaginationDto) {
    const [users, count] = await User.findAndCount({
      take: pagination.limit,
      skip: pagination.offset,
    });
    return successResponse({ users: users, totalRows: count });
  }
  ...
  async findOne(
    ...
    return await User.findOne({
      ...
      where: [{ email: data }, { id: data }, { phone_number: data }],
    });
  }
  ...
  async creditUserWallet(
    ...
    onesignal.sendNotification(
      'Wallet Account Credited Successfully',
      `You wallet has been credited with ${user.currency_symbol}${amount}`,
      user.id,
      {},
    );
  ...
  async updateUser(
    ...
    if (profile_picture)
      uploaded_profile_picture = await useB2FileUpload(
    ...
    if (
      await User.findOne({
        where: [
          { email: email, id: Not(user.id), phone_number: phone_number },
          {
            email: email,
            id: Not(user.id),
          },
    ...
    if (country) {
      ...
      user.currency_symbol =
        country === SUPPORTED_COUNTRIES.USA.name
          ? SUPPORTED_COUNTRIES.USA.symbol
          : SUPPORTED_COUNTRIES.NIGERIA.symbol;
      user.currency =
        country === SUPPORTED_COUNTRIES.USA.name
          ? SUPPORTED_COUNTRIES.USA.code
          : SUPPORTED_COUNTRIES.NIGERIA.code;
      user.city = city;
    }
  ...
  async changePassword(changePassword: ChangePassword, user: User) {
    ...
    if (confirm_password !== new_password)
      returnErrorResponse('password dont match');
    const userData = await User.findOne({
      where: { id: user.id },
      select: { email: true, password: true },
    });
    if (!(await this.comparePassword(userData.password, password)))
      returnErrorResponse('Invalid current password');
