...
export class TransactionsService {
  ...
  webhookServiceHandler(payload: any): void {
    ...
    if (
      payload.event === PAYSTACK_WEBHOOK_EVENTS.TRANSFER_SUCCESS ||
      payload.event === PAYSTACK_WEBHOOK_EVENTS.TRANSFER_FAILED ||
      payload.event === PAYSTACK_WEBHOOK_EVENTS.TRANSFER_REVERSED
    )
      this.handleTransfersViaWebhook(payload);
  }
  ...
  async handleTransfersViaWebhook(payload: any): Promise<void> {
    ...
    const transaction = await this.findOne(transfer_id, [
      'transfer_id',
      'user_id',
      'status',
      'id',
    ...
    let user;
    if (transaction)
      user = await this.userService.findOne(transaction.user_id, [
        'wallet_balance',
        'id',
      ]);
    if (transaction && user) {
      switch (payload.event) {
        ...
        case PAYSTACK_WEBHOOK_EVENTS.TRANSFER_FAILED ||
          PAYSTACK_WEBHOOK_EVENTS.TRANSFER_REVERSED:
  ...
  async fundWallet(data: any) {
    ...
    if (user) {
      const card =
        data.authorization.channel === TRANSACTION_METHOD.CARD
          ? await this.saveCard(data, user)
          : null;
      await this.create({
  ...
  async verifyTransaction(
    ...
    const verified = await verifyTransaction(
      verifyTransactionDto.payment_reference,
      user,
    );
  ...
  async initializeTransactionDto(
    user: User,
  ...
  async initialiseStripeTransaction(
    user: User,
    initializeTransactionDto: InitializeStripeTransactionDto,
  ): Promise<SuccessResponseType> {
    ...
    if (!customerId) {
      ...
      if (!customer)
        returnErrorResponse('Could not initialize stripe payment gateway');
    ...
    const response =
      transaction_type === 'add_card'
        ? await stripe.createSetupIntent(customerId)
        : await stripe.createCheckout('Fund Your Wallet', amount, {
  ...
  async findOne(
    ...
    return await Transaction.findOne({
      ...
      select:
        columns_to_load && columns_to_load.length
          ? columns_to_load
          : ['id', 'user_id', 'reference'],
    });
  }

  async saveCard(data, user) {
    let card = await Card.findOne({
      where: { signature: data.authorization.signature, user_id: user.id },
    });
    if (!card) {
      card = new Card();
    }
