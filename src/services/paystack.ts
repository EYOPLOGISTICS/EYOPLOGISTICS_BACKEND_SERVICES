...
const usePaystackService = () => {
  ...
  const verifyTransaction = async (reference: string, user: User) => {
    try {
      ...
      if (response.data.status) {
        ...
        if (
          response.data.data.authorization.channel === TRANSACTION_METHOD.CARD
        )
          await saveCard(response.data.data, user);
        return true;
      }
      return false;
    } catch (e) {
  ...
  const initializeTransaction = async (
    ...
    try {
      ...
      const data = {
        ...
        transaction_charge: charge * 100,
        metadata,
        channels:
          payment_channels && payment_channels.length
            ? payment_channels
  ...
  const verifyBankAccount = async (
    ...
    try {
      ...
      'Could not resolve account name. Check parameters or try again.'
        ? returnErrorResponse(e.response.data)
        : returnErrorResponse(
            'Oops, seems our network is poor please try again',
          );
    }
  };
  ...
  const chargeCard = async (
    ...
    try {
      ...
      if (response.data.status === true)
        return await verifyTransaction(response.data.data.reference, user);
      return false;
    } catch (e) {
      console.log(e);
      return false;
  ...
  const authenticate = (req_body: any, paystack_signature: any) => {
    ...
    const hash = crypto
      ...
      .update(JSON.stringify(req_body))
      .digest('hex');
