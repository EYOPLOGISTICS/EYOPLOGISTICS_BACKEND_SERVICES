...
const stripe = require('stripe')(
  'sk_test_51PXnWmRuYEIazKf9pR04SAcOuB7hvVHOWU6F1u08LHUjHPylcRpCyzWrmeBvDq9qBvFHWrWTXytBBGj7y9zSwn7400kI8AGnlV',
);
const googleMapServices = useGoogleMapServices();

const { verifyBankAccount, getNigerianBanks, authenticate } =
  usePaystackService;
...
export class AppController {
  ...
  async uploadMultipleFiles(
    @UploadedFiles(new ParseFilePipe({ fileIsRequired: true })) files: any,
  ): Promise<SuccessResponseType> {
    const tempFileArray = [];
    for (const file of files) {
      const uploaded_url = await useB2FileUpload(
        file.originalname,
        file.buffer,
      );
