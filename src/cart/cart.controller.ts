...
export class CartController {
  ...
  async getCart(@AuthUser() user: User) {
    const cart = await this.cartService.getCartMeta(user.id);
    return successResponse({
      cart: cart ? await this.cartService.getCartFull(cart.id) : null,
    });
  }
  ...
  removeProduct(
    ...
    return this.cartService.removeProductInCart(productId, user);
  }
}
